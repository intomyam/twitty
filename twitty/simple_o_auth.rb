#Copyright (c) 2009 shibason(http://d.hatena.ne.jp/shibason/)
...
class Twitty::SimpleOAuth
  ...
  def self.query_to_s(query = {})
    query = '?' + query.collect{|array| "#{array.first}=#{array.last}"}.join('&')
  end
